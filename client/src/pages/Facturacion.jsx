...
function Facturacion({ socket }) {
  ...
  const [columnWidths, setColumnWidths] = useState({
    codigo: getInitialColumnWidth('codigoBarras', 140),
    descripcion: getInitialColumnWidth('descripcion', 300),
    cantidad: getInitialColumnWidth('cantidad', 80),
    precio: getInitialColumnWidth('precio', 100),
    descuento: getInitialColumnWidth('descuento', 80),
    subtotal: getInitialColumnWidth('subtotal', 100)
  })
  ...
  // Efecto para manejar el redimensionamiento con el mouse
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      // Aplicar límites según la columna
      let minWidth = 60
      let maxWidth = 600

      if (resizingColumn === 'descripcion') {
        maxWidth = 1000
        minWidth = Math.max(100, columnWidths.cantidad) // Descripción no puede ser menor que cantidad
      }

      // Limitar código de barras a máximo 140px (suficiente para 20 caracteres)
      if (resizingColumn === 'codigo') {
        maxWidth = 200
        minWidth = 60
      }
      ...
      setColumnWidths(prev => ({
        ...prev,
        [resizingColumn]: Math.round(clampedWidth)
      }))
    }
